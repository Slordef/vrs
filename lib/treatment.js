...
module.exports = async function (infos) {
    ...
    const ver = `${moment.utc().format('YYYYMMDDHHmmss')}`
    let newElem = {dir: ver, current: true}
    const dir = `${path}/versions/${ver}`
    ...
    clearInterval(twirl)
    console.log(chalk.underline.green("Ver : New Version"))
    console.log(chalk.green(`current : ${dir}`))
}

// TODO refacto complete !!!
function checkForRepo (infos) {
    ...
    if (infos.repo.build) {
        return new Promise((resolve, reject) => {
            ...
            let e_build = `npm install && npm run build`
