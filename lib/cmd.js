...
module.exports = function (params) {
    ...
    if(params.has(['--no-process', '-np'])){
        ...
        const child = spawn('vers', list, { cwd: process.cwd(), detached:true, stdio:'ignore', shell: (process.platform === 'win32'), windowsHide: true })
        child.unref()
        console.log(chalk.green(`Vers cmd with no progress running`))
        return
    }
