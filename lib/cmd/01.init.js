...
function fnInit (options) {
    ...
    inquirer.prompt(questions(data)).then(async answers => {
        let service = data
        const { name, config, useFile, file, conserve, before, after, useRepo, logs } = answers
        const repo = {
            path: answers.repo,
            build: answers.build,
        ...
        const newService = { name, config, useFile, file, conserve, useRepo, repo, path:pwd, before, after, versions: [], logs }
        service = format(service, newService)
        init(service)
        await treatment(service, 'init')
        console.log(chalk.underline.green(`VRS initialized for :`))
        console.log(chalk.green(`name: ${service.name}`))
        console.log(chalk.green(`path: ${service.path}`))
    })
}
...
const init = function (vrs) {
    ...
    if (!fs.existsSync(path + `/current`)) {
        ...
        if (!fs.existsSync(path + dir)) fs.mkdirSync(path + dir)
        fs.symlinkSync(path + dir, path + `/current`)
    } else {
    ...
    if (service.config.length && !fs.existsSync(`${path}/${service.config}`)) fs.writeFileSync(`${path}/${service.config}`, configPlaceholder)
